    ...
    if (!data) {
        ...
        return (
            ...
            !data ?
                ...
                <>
                    <h1 className={`${styles.header} text text_type_main-large  pt-15 pb-5`}>Лента заказов</h1>
                    <main className={styles.main}>
                        <div className={styles.FeedOrders}>
                            {orders.map((order) => <FeedOrders order={order} />)}
                        </div>
                            ...
                            <div className={styles.OrdersContainer}>
                                <div className={styles.DoneOrders}>
                                    ...
                                    <ul className={styles.OrdersList}>
                                        {orders.filter((order) => order.status === 'done').slice(0, 10).map((order) => order.status === 'done' && <li className={`${styles.DoneList} text text_type_digits-default text_color_success mb-2`} key={order._id} >{order.number}</li>)}
                                ...
                                <h2 className="text text_type_main-medium mb-6">В работе:</h2>
                                    <ul className={styles.OrdersList}>
                                        ...
                                        {orders.filter((order) => order.status === 'pending').slice(0, 10).map((order) => order.status === 'pending' && <li className={`${styles.ProcessList} text text_type_digits-default text_color_success mb-2`} key={order._id}>{order.number}</li>)}
                                    </ul>
                                </div>
                            </div>
